...
**方向层:低。** "把荣格八维接到 LLM 上"这件事已经有人做,而且做得比这份文档更实证。

最相关的是 2026 年 7 月的 *The Geometry of Personality: Activation Steering with Jungian Cognitive Functions*(Liu Zai, Yumeng Wang, Junchen Fu, Joemon M. Jose;arXiv:2607.20803)。它的做法与结论:

- 建了一套荣格评估协议和一个 NarrationDB 数据集(2100+ 条角色扮演叙述,132 条陈述、Likert 量表评分),在 Llama-3.1-8B 上抽取并评估激活方向。
- **结论一**:全部八个功能都能被单调操控——steering 强度与打分呈单调关系,不是有没有效果的问题,是幅度可调。
- **结论二**:人格信息集中在中间层。
- **结论三**:steering 方向之间呈现结构化的几何关系,且这个结构**与理性/非理性之分一致**。
- **结论四**:有效的多维 steering 方向**无法由单功能方向线性组合恢复**。

这四条对本架构的关系,值得逐条说清:

| 它的结论 | 与本架构的关系 |
|---|---|
| 单调可控 | 中性。证明八个概念在模型里可分离,但不证明它们是计算机制 |
| 中间层集中 | 中性偏支持,契合 encode–compute–decode 的分层观 |
| 理性/非理性的几何区分 | **直接支持第一条公理**,而且是独立得到的 |
| 多维不可线性恢复 | **既是支持也是新约束**——功能栈不是八个方向的加权和,这正是第 3 节"知觉互斥、赋值可加"预测的东西,但也逼出了第 8 节第 6 条那个更锐利的假设 |

所以它不是"不值一提",恰恰相反:它是目前唯一一份能给本架构提供实证支点的工作,第三条结论几乎是替第一条公理做的实验。

它的**局限**同样要说清,而且这个局限决定了两份工作不构成竞争。NarrationDB 是角色扮演叙述,评估靠量表自评。这条链路测的是"模型能不能表征并演出一个 Ni 主导的角色",不是"模型内部有没有一个 Ni 式的计算过程"。换句话说,抽出来的很可能是**人格描述空间里的方向**,而不是**认知机制**。这是激活操控类工作的通病,不是这篇的疏忽——但它意味着结论三对本架构的支持力度要打折:两类方向的几何差异,也可能只是因为训练语料里描述"思维型"和"描述感觉型"的措辞本来就分属两簇。

而本架构的处境正好相反:它讲的是机制,但**一个实验都没有**。所以诚实的说法是——它做了测量,我们做了解释,两边各缺对方那一半。

"荣格阴影 ↔ Waluigi 效应"的对应在 2023 年起的博客与评论层面也已流传,那一层基本没有形式内容。
...
**有意合作请联系。** 无论是想把第 8 节的可检验推论真的跑一遍、手上已有相关的 activation steering 数据、还是对形式化本身有异议或有更好的版本,都欢迎通过 [GitHub Issues](https://github.com/zhuoyuanmao/typology-as-inference/issues) 提出,或直接发起 Pull Request。

**受到启发并形成论文,请引用本仓库。** 如果这里的任何构造(尤其是第 6 节"无意识 = 在被拒候选流上训练的对偶策略",以及第 8 节的推论 3、4、6)对你的工作有帮助,烦请在文中注明出处:
